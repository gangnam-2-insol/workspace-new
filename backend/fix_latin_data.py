#!/usr/bin/env python3
"""
기존 데이터베이스의 라틴어 데이터를 실제적인 한국어 내용으로 교체하는 스크립트
"""
import asyncio
import random
import re
...
async def fix_latin_data():
    """기존 데이터베이스의 라틴어 데이터를 실제적인 내용으로 교체"""
    try:
        # MongoDB 연결
        client = AsyncIOMotorClient('mongodb://localhost:27017/hireme')
        db = client.hireme
        
        print("🔍 라틴어 데이터 검사 및 교체 시작...")
        ...
        for applicant in applicants:
            ...
            for pattern in latin_patterns:
                if (re.search(pattern, career_history.lower()) or 
                    re.search(pattern, growth_background.lower()) or
                    re.search(pattern, motivation.lower()) or
                    re.search(pattern, analysis_result.lower())):
                    has_latin = True
                    break
            ...
            if has_latin:
                ...
                # 새로운 내용 생성
                new_career = generate_realistic_career_history(position, experience)
                new_growth = generate_realistic_growth_background(position, experience)
                new_motivation = generate_realistic_motivation(position, "회사")
                new_analysis = generate_realistic_analysis_result(position, experience)
                ...
                result = await db.applicants.update_one(
                    ...
                    {
                        '$set': {
                            'careerHistory': new_career,
                            'growthBackground': new_growth,
                            'motivation': new_motivation,
                            'analysisResult': new_analysis,
