...
async def upload_multiple_documents(
    ...
    try:
        # 최소 하나의 파일 또는 GitHub URL이 필요
        if not resume_file and not cover_letter_file and not portfolio_file and not github_url:
            raise HTTPException(status_code=400, detail="최소 하나의 문서 파일 또는 GitHub URL이 필요합니다")
        ...
        # 4. GitHub URL 처리 (기존 지원자에 연결)
        if github_url:
            print(f"🔗 GitHub URL 처리 시작: {github_url}")
            ...
            try:
                ...
                if len(path_parts) >= 1:
                    ...
                    print(f"📋 추출된 GitHub 사용자명: {github_username}")
                    
                    # GitHub 분석 수행 (github.py의 함수 사용)
                    print(f"🤖 GitHub 분석 중...")
                    # 실제 GitHub 분석을 수행하는 대신 간단한 정보만 저장
                    github_result = {
                    ...
                    if applicant_id:
                        ...
                        existing_applicant = mongo_saver.mongo_service.get_applicant_by_id_sync(applicant_id)
                        if existing_applicant:
                            # 기존 지원자에 GitHub 정보 추가
                            update_data = {"github_url": github_url, "github_username": github_username}
                            mongo_saver.mongo_service.update_applicant_sync(applicant_id, update_data)
                            ...
                            applicant_data = _build_applicant_data(name, email, phone, {}, job_posting_id)
                    else:
                        applicant_data = _build_applicant_data(name, email, phone, {}, job_posting_id)
                        # 새 지원자 생성
                        new_applicant = mongo_saver.mongo_service.create_or_get_applicant(applicant_data)
                    ...
                    print(f"✅ GitHub URL 처리 완료: {github_username}")
                    print(f"🔗 GitHub URL: {github_url}")
                    
                else:
                    print(f"❌ GitHub URL 형식이 올바르지 않습니다: {github_url}")
                    
            except Exception as e:
                import traceback
                error_traceback = traceback.format_exc()
                print(f"❌ GitHub URL 처리 실패: {e}")
                print(f"🔍 GitHub 에러 상세 정보:")
                print(error_traceback)
                # GitHub 처리 실패는 전체 처리를 중단하지 않음
                results["github"] = {
                    "message": f"GitHub URL 처리 실패: {str(e)}",
                    "error": True
                }
