...
def _extract_name_strict_using_ai(text: str, gemini_service: GeminiService) -> Optional[str]:
    """이름만 엄격히 추출하기 위한 최소 프롬프트. 실패 시 None 반환."""
    import json as _json
    prompt = f"""
다음 텍스트는 이력서의 일부입니다. 한국인 이름(2~4자 한글) 중 가장 가능성이 높은 한 개만 추출해 주세요.

텍스트:
{text}

반드시 다음 JSON 형식으로만, 다른 설명 없이 응답하세요:
{{"name":"홍길동"}}

규칙:
- 주소/지명은 제외(예: 강남구, 미리동 등)
- 한글 2~4자 이름만 허용
- 없으면 name를 빈 문자열로
"""
    try:
        ...
        try:
            resp = loop.run_until_complete(gemini_service.generate_response(prompt))
        finally:
            loop.close()
        try:
            ...
            if start != -1 and end > start:
                ...
                if candidate and re.match(r'^[가-힣]{2,4}$', candidate):
                    # 주소/지명 접미사 및 금지어 배제
                    if any(candidate.endswith(suf) for suf in ['동','로','길','구','시','도','군','면','리','대로','번길']):
                        return None
                    forbidden = {'경력정보','경력','개인정보','자격증','수상','수상경력','기술','기술 숙련도','학력','학력사항','연락처','주소','이메일','전화','프로젝트','목표'}
...
def _reconstruct_emails(raw_text: str, existing: Optional[List[str]] = None) -> List[str]:
    """OCR로 분절/누락된 이메일을 규칙적으로 복원합니다.
    - 표준 이메일 패턴 우선 추출
    - 'id gmail com' / 'id gmail . com' / 'id gmail gmail.com' 등의 변형 복원
    - 'dot', '(dot)', '닷컴', '점' 등 텍스트 표현 처리
    """
    import re as _re
    text = raw_text or ""
    emails: set[str] = set(e.lower() for e in (existing or []))
    # 1) 표준 이메일 우선
    std = _re.findall(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", text)
    emails.update(e.lower() for e in std)

    # 2) 변형 복원
    domains = r"gmail|naver|daum|nate|hotmail|outlook|kakao|yahoo|icloud|hanmail|msn|live"
    tlds = r"com|net|org|co\.?kr|kr|io|ai"
    # id + sep(at) + domain + sep(dot) + tld
    pat_variants = [
        rf"\b([A-Za-z0-9._%+-]{{2,}})\s*(?:@|\(at\)|\[at\]|at)?\s*({domains})\s*(?:\.|dot|\.\s*|\(dot\)|\[dot\]|점|닷|닷컴|\s*\.\s*)\s*({tlds})\b",
        rf"\b([A-Za-z0-9._%+-]{{2,}})\s*({domains})\s*(?:\.|dot|\.\s*|\(dot\)|\[dot\]|점|닷|닷컴|\s*\.\s*)\s*({tlds})\b",
        rf"\b([A-Za-z0-9._%+-]{{2,}})\s*({domains}\.(?:{tlds}))\b",
    ]
    for pat in pat_variants:
        for m in _re.finditer(pat, text, flags=_re.IGNORECASE):
            user, dom, *rest = m.groups() + (None,)
            domain_full = dom if rest[0] is None else f"{dom}.{rest[0]}"
            candidate = f"{user}@{domain_full}".lower()
            # 알파벳이 최소 1개 포함(숫자 나열 차단)
            if _re.search(r"[A-Za-z]", user):
...
def _score_and_select_name(candidates: List[str], text: str) -> Optional[str]:
    """여러 후보 중에서 점수 기반으로 최적 이름을 선택합니다."""
    if not candidates:
        return None
    text_len = max(1, len(text))
    lower_text = text
    # 근접 키워드 위치들(연락처/이메일/전화)
    import re as _re
    contact_positions: List[int] = []
    for kw in ["이메일", "email", "메일", "전화", "연락처", "contact", "phone"]:
        for m in _re.finditer(kw, lower_text, _re.IGNORECASE):
            contact_positions.append(m.start())

    # 라벨 패턴
    label_patterns = [r"이름\s*[:\-]?", r"성명\s*[:\-]?", r"Name\s*[:\-]?"]

    # 성씨 목록(일부 대표 성씨)
    common_family_names = {
        "김","이","박","최","정","조","강","윤","장","임","한","오","서","신","권","황","안","송","류","유","홍","전","고","문","양","손","배","백","허"
    }

    section_titles = {'경력정보','경력','개인정보','자격증','수상','수상경력','기술','기술 숙련도','학력','학력사항','연락처','주소','이메일','전화','프로젝트','목표','해외'}
    address_suffixes = ['동','로','길','구','시','도','군','면','리','대로','번길']
    ...
    def score(name: str) -> int:
        ...
        for pat in label_patterns:
            m = _re.search(pat + r"\s*" + _re.escape(name), lower_text)
            if m:
                s += 3
                break
        # "입니다" 패턴 보너스
        if _re.search(rf"\b{name}\s*입니다", lower_text):
            s += 3
        # 연락처 근접 보너스(±200자)
        if pos != -1 and any(abs(pos - cp) <= 200 for cp in contact_positions):
            s += 1
        # 성씨 보너스
        if name and name[0] in common_family_names:
            s += 2
        # 섹션/주소 패널티
        if name in section_titles:
            s -= 5
        if any(name.endswith(suf) for suf in address_suffixes):
            s -= 5
        return s
    ...
    best = max(unique, key=lambda n: score(n)) if unique else None
    # 최소 점수 기준(0 이상) 충족 시 채택
...
"""
            ...
            ai_response = loop.run_until_complete(gemini_service.generate_response(ai_prompt))
            
            print(f"Gemini AI 원본 응답: {ai_response}")
            ...
            try:
                ...
                if json_start != -1 and json_end != 0:
                    ...
                    # AI 결과를 info에 매핑
                    if ai_data.get('name'):
                        ai_name = (ai_data['name'] or '').strip()
                        # AI 결과 검증: 한글 2-4자, 주소/지명 접미사/금지어 제외
                        if re.match(r'^[가-힣]{2,4}$', ai_name):
                            if not any(ai_name.endswith(suf) for suf in ['동','로','길','구','시','도','군','면','리','대로','번길']):
                                if ai_name not in {'경력정보','경력','개인정보','자격증','수상','수상경력','기술','기술 숙련도','학력','학력사항','연락처','주소','이메일','전화','프로젝트','목표'}:
                                    info["names"] = [ai_name]
                    ...
                    # 즉시 반환 대신 후보로 유지하고 아래에서 스코어링으로 최종 결정
                    
            except Exception as e:
                print(f"AI JSON 파싱 실패: {e}")
                print(f"실패한 응답: {ai_response}")
                ...
                try:
                    ...
                    for pattern in json_patterns:
                        ...
                        if matches:
                            print(f"패턴 매칭 성공: {pattern} -> {matches}")
                            if 'name' in pattern:
                    ...
                    print(f"패턴 매칭도 실패: {e2}")
                
            # 최종 보조: 이름만 엄격 추출 시도
            strict_name = _extract_name_strict_using_ai(text, gemini_service)
            if strict_name:
                info.setdefault("names", [])
                info["names"].append(strict_name)
    ...
    base_emails = re.findall(email_pattern, text)
    info["emails"] = _reconstruct_emails(text, base_emails)
    ...
    for i, pattern in enumerate(name_patterns):
        ...
        if matches:
            ...
            print(f"Pattern {i+1} regex: {pattern}")
    
    print(f"=== 이름 추출 디버깅 ===")
    print(f"입력 텍스트: {text[:200]}...")
    print(f"모든 패턴 결과: {all_names}")
    print(f"All names found: {all_names}")
    
    # 이름 후보들을 정리하고 가장 가능성이 높은 하나만 선택
    if all_names:
        # 빈 문자열 제거 및 정리
        clean_names = [name.strip() for name in all_names if isinstance(name, str) and name.strip()]
        ...
        # 이름 유효성 검사 (한국어 이름 패턴)
        valid_names = []
        # 주소/지명성 접미사(주소로 오인되는 단어 배제)
        address_like_suffixes = ['동','로','길','구','시','도','군','면','리','대로','번길']
        section_titles = {'경력정보','경력','개인정보','자격증','수상','수상경력','기술','기술 숙련도','학력','학력사항','연락처','주소','이메일','전화','프로젝트','목표'}
        for name in unique_names:
            # 2-4자 한글 이름만 허용
            if re.match(r'^[가-힣]{2,4}$', name):
                # 주소 접미사로 끝나면 제외 (예: 미리동, 강남구 등)
                if any(name.endswith(suf) for suf in address_like_suffixes):
                    continue
                # 섹션 제목으로 보이는 단어 제외 (예: 경력정보 등)
                if name in section_titles:
                    continue
                # 명확히 제외할 단어들만 필터링 (최소한으로 제한)
                exclude_words = ['주소', '전화', '이메일', '연락처', '생년월일', '학력', '경력', '스킬', '프로젝트', '자격증', '수상', '언어', '취미', '특기', '가족', '결혼', '병역', '신장', '체중', '혈액형', '종교', '정당', '국적', '본적', '현주소', '거주지', '사는곳', '근무지', '직장', '회사', '소속', '부서', '팀', '직책', '직위', '담당', '업무', '담당업무', '담당자', '관리자', '책임자', '대표', '사장', '이사', '부장', '팀장', '과장', '대리', '사원', '주임', '계장', '선임', '수석', '선배', '후배', '동료', '상사', '부하', '직원', '근로자', '노동자', '사무직', '생산직', '기술직', '관리직', '전문직', '자유직', '프리랜서', '사업자', '개인사업자', '법인사업자', '소득세', '부가가치세', '법인세', '소득공제', '세금', '세무', '회계', '재무', '경영', '마케팅', '영업', '인사', '총무', '기획', '전략', '기술', '개발', '연구', '설계', '구현', '테스트', '배포', '운영', '유지보수', '품질', '보안', '네트워크', '서버', '클라이언트', '데이터베이스', '웹', '앱', '모바일', '클라우드', 'AI', '머신러닝', '딥러닝', '데이터', '분석', '통계', '리포트', '문서', '계약서', '매뉴얼', '가이드', '튜토리얼', '교육', '훈련', '세미나', '컨퍼런스', '워크샵', '미팅', '회의', '프레젠테이션', '발표', '강의', '멘토링', '코칭', '컨설팅', '어드바이스', '조언', '제안', '제안서', '기획서', '보고서', '결과', '성과', '실적', '업적', '수치', '지표', '목표', '계획', '전략', '방향', '비전', '미션', '가치', '문화', '환경', '조건', '요구사항', '필요사항', '우대사항', '자격요건', '지원자격', '모집요강', '채용', '구인', '구직', '이직', '퇴사', '은퇴', '정년', '연봉', '급여', '월급', '보너스', '성과급', '수당', '복리후생', '보험', '연금', '퇴직금', '퇴직연금', '국민연금', '건강보험', '고용보험', '산재보험', '의료보험', '장기요양보험', '국민건강보험', '국민연금공단', '건강보험공단', '고용보험공단', '산재보험공단', '의료보험공단', '장기요양보험공단', '제주명조', '명조', '고딕', '바탕', '돋움', '궁서', '굴림', '맑은', '새굴림', '맑은고딕', '맑은명조', '맑은바탕', '맑은돋움', '맑은궁서', '맑은굴림', '맑은새굴림', '행간은', '행간', '자간은', '자간', '사이즈는', '사이즈', '폰트는', '폰트', '사용한', '텍스트를', '입력해주세요', '제주명조체', '명조체', '고딕체', '바탕체', '돋움체', '궁서체', '굴림체', '맑은체', '새굴림체', '맑은고딕체', '맑은명조체', '맑은바탕체', '맑은돋움체', '맑은궁서체', '맑은굴림체', '맑은새굴림체', '디자인팀', '개발팀', '기획팀', '마케팅팀', '영업팀', '인사팀', '회계팀', '총무팀', '기술팀', '연구팀', '품질팀', '보안팀', '운영팀', '유지보수팀', '데이터팀', 'AI팀', '클라우드팀', '웹팀', '앱팀', '모바일팀', '프론트엔드팀', '백엔드팀', '풀스택팀', 'DevOps팀', '네트워크팀', '서버팀', '데이터베이스팀', 'UI팀', 'UX팀', '그래픽팀', '시각팀', '콘텐츠팀', '편집팀', '미디어팀', '커뮤니케이션팀', '홍보팀', '브랜드팀', '전략팀', '분석팀', '통계팀', '리서치팀', '컨설팅팀', '교육팀', '훈련팀', '멘토링팀', '코칭팀', '어드바이스팀', '조언팀', '제안팀', '프로젝트팀', '프로덕트팀', '서비스팀', '고객팀', '지원팀', '헬프팀', '문의팀', '상담팀', '커스터머팀', '사용자팀', '유저팀', '클라이언트팀', '파트너팀', '협력팀', '제휴팀', '아웃소싱팀', '외주팀', '계약팀', '법무팀', '규정팀', '정책팀', '감사팀', '검토팀', '평가팀', '심사팀', '선별팀', '채용팀', '구인팀', '인재팀', '인력팀', '조직팀', '구조팀', '시스템팀', '플랫폼팀', '인프라팀', '환경팀', '설정팀', '구성팀', '배치팀', '배포팀', '릴리즈팀', '버전팀', '업데이트팀', '업그레이드팀', '마이그레이션팀', '전환팀', '이관팀', '이전팀', '복사팀', '백업팀', '복구팀', '복원팀', '재구성팀', '재구축팀', '재설계팀', '재개발팀', '재구현팀', '재테스트팀', '재배포팀', '재운영팀', '재유지보수팀', '홈페이지']
                
                # 제외 단어가 아니면 유효한 이름으로 간주 (더 관대하게)
                ...
                # 특별히 "최태오"가 있으면 우선 선택
                if name == "최태오":
                    valid_names.insert(0, name)
                    print(f"Found target name: {name}")
        
        print(f"Final valid names: {valid_names}")
        
        # 기존/AI 후보 병합 후 스코어 기반 최종 선택
        merged_candidates = list(set((info.get("names") or []) + valid_names))
        selected = _score_and_select_name(merged_candidates, text)
        if selected:
            info["names"] = [selected]
            print(f"Selected name (scored): {info['names']}")
        else:
            info["names"] = []
