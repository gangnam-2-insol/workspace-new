...
# 검색 결과 상세 페이지 라우터 추가
try:
    from routers.search_results_router import router as search_results_router
except ImportError:
    search_results_router = None
...
try:
    ...
    # 검색 결과 상세 페이지 라우터 등록
    if search_results_router:
        app.include_router(search_results_router, prefix="/api", tags=["search-results"])
    else:
        print("⚠️ 검색 결과 라우터를 불러올 수 없습니다.")
except ImportError:
    pass
...
@app.post("/api/send-individual-mail")
async def send_individual_mail(request: Request):
    """개별 메일 발송"""
    try:
        import smtplib
        from email.mime.multipart import MIMEMultipart
        ...
        data = await request.json()
        print(f"📧 [DEBUG] 개별 메일 발송 요청: {data}")

        recipient = data.get("recipient")
        template = data.get("template", "불합격 통지")
        input_text = data.get("input_text", "")

        print(f"📧 [DEBUG] 수신자: {recipient}, 템플릿: {template}")

        if not recipient:
            print(f"📧 [DEBUG] 수신자 이메일 누락 - recipient: {recipient}")
        ...
        if not mail_settings:
            print(f"📧 [DEBUG] 메일 설정을 찾을 수 없음 - _id: default")
            raise HTTPException(status_code=400, detail="메일 설정이 필요합니다.")
        ...
        if not mail_templates:
            print(f"📧 [DEBUG] 메일 템플릿을 찾을 수 없음 - _id: default")
            raise HTTPException(status_code=400, detail="메일 템플릿이 필요합니다.")

        # 템플릿 선택
        print(f"📧 [DEBUG] 템플릿 선택: {template}")
        if template == "불합격 통지":
            template_data = mail_templates.get("rejected", {})
        elif template == "합격 통지" or template == "합격 메일 템플릿":
            template_data = mail_templates.get("passed", {})
        elif template == "면접 안내":
            template_data = mail_templates.get("document_passed", {})
        elif template == "최종 합격":
            template_data = mail_templates.get("final_passed", {})
        elif template == "서류 불합격":
            template_data = mail_templates.get("rejected", {})
        else:
            # 기본값으로 rejected 템플릿 사용
            template_data = mail_templates.get("rejected", {})
            print(f"📧 [DEBUG] 알 수 없는 템플릿 '{template}', 기본값 'rejected' 사용")

        print(f"📧 [DEBUG] 선택된 템플릿 데이터: {template_data}")

        subject = template_data.get("subject", "안내 메일")
        content = template_data.get("content", "안내 메일입니다.")

        print(f"📧 [DEBUG] 메일 제목: {subject}")
        print(f"📧 [DEBUG] 메일 내용: {content}")

        # 변수 치환
        content = content.format(
            applicant_name="지원자",
            job_posting_title="채용공고",
            company_name="회사"
        )
        ...
        print(f"📧 [DEBUG] 메일 객체 생성 완료 - From: {msg['From']}, To: {msg['To']}")

        # SMTP 서버 연결 및 메일 발송
        smtp_port = mail_settings.get('smtpPort', 587)
        smtp_server = mail_settings.get('smtpServer', 'smtp.gmail.com')

        print(f"📧 [DEBUG] SMTP 서버: {smtp_server}:{smtp_port}")
        ...
        print(f"📧 [DEBUG] 개별 메일 발송 성공: {recipient}")
        return {
            "success": True,
            "message": f"메일이 {recipient}로 성공적으로 발송되었습니다.",
            "data": {
                "recipient": recipient,
                "template": template,
                "subject": subject,
        ...
        print(f"📧 [DEBUG] 개별 메일 발송 오류: {str(e)}")
        raise HTTPException(status_code=500, detail=f"개별 메일 발송 중 오류가 발생했습니다: {str(e)}")
