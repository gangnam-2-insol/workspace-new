...
5. Pinecone 인덱스 생성:
Pinecone 콘솔에서 다음 설정으로 `resume-vectors` 인덱스를 생성하세요:
- **이름**: `resume-vectors`
- **차원**: `1536` (OpenAI text-embedding-3-small 모델 호환)
- **메트릭**: `cosine`
...
{
  "success": true,
  "message": "LangChain 하이브리드 검색 완료: 'React 개발 경험 3년 이상'",
  "data": {
    "query": "React 개발 경험 3년 이상",
...
graph TD
    ...
    E --> F[임베딩 벡터 생성<br>OpenAI text-embedding-3-small]
    F --> G[Pinecone 벡터 저장]
    G --> H[메타데이터 연결 완료]
...
3. **벡터 변환**: **OpenAI text-embedding-3-small**을 사용하여 청크별 임베딩 생성
...
- **특징**: 한국어 및 다국어 지원, 1536차원 벡터
- **장점**: 최신 OpenAI 모델의 향상된 임베딩 품질
- **폴백**: SentenceTransformer `paraphrase-multilingual-MiniLM-L12-v2` (OpenAI 실패 시)
- **LangChain 호환**: LangChain EnsembleRetriever와 동일한 모델 사용
...
### 검색 방법별 특징 및 융합 (LangChain 적용)
- **LangChain 벡터 검색**: PineconeVectorStore 기반 의미적 유사도 검색
- **기존 KeywordSearchService**: BM25 + 멀티필드 + 퍼지매칭 (벡터 필드 불필요)
- **수동 하이브리드 융합**: 벡터 + 키워드 결과를 중복 제거 후 수동 결합
- **Elasticsearch 호환성**: 기존 스키마 변경 없이 LangChain 통합
- **폴백 지원**: LangChain 실패 시 기존 수동 방식으로 자동 전환
...
### 융합 점수 계산 방식 (LangChain 기반)
```
# LangChain 수동 하이브리드 방식
최종점수 = (LangChain벡터점수 × 0.5) + (키워드점수 × 0.5)

# 폴백 시 기존 방식
최종점수 = (벡터점수 × 0.5) + (Elasticsearch점수 × 0.5)
```
- **LangChain 벡터 우선**: PineconeVectorStore 기반 벡터 검색
- **기존 키워드 서비스**: KeywordSearchService로 Elasticsearch 호환성 유지
- **수동 융합**: 벡터 + 키워드 결과를 중복 제거 후 수동 결합
- **Elasticsearch 점수**: 멀티필드 검색 + 토큰 매칭 조합
- **벡터 점수**: Pinecone 코사인 유사도
- **결과 정렬**: 최종 융합 점수 기준 내림차순
- **검색 방법 표시**: vector, keyword 매칭 여부 표시
- **안정성**: LangChain 실패 시 기존 방식으로 자동 폴백

## LangChain 하이브리드 검색 시스템 🆕🔥

### LangChain 수동 하이브리드 검색 기반 시스템
- **LangChain 라이브러리**: 최신 LangChain 프레임워크 적용
- **수동 하이브리드**: 벡터 + 키워드 검색을 수동으로 융합 (호환성 우선)
- **Elasticsearch 호환**: 기존 스키마 변경 없이 LangChain 통합
- **자동 폴백**: LangChain 실패 시 기존 수동 방식으로 자동 전환
- **성능 향상**: 더 정확한 검색 결과와 향상된 관련성 점수

### LangChain 컴포넌트 구성
- **OpenAI Embeddings**: `text-embedding-3-small` 모델 사용 (1536차원)
- **PineconeVectorStore**: LangChain 기반 벡터 스토어
- **KeywordSearchService 호환**: 기존 Elasticsearch 구조와 호환 (벡터 필드 불필요)
- **수동 하이브리드 검색**: 벡터 + 키워드 결과를 수동으로 융합 (중복 제거 포함)
- **통합된 임베딩**: 기존 EmbeddingService와 동일한 OpenAI 모델 사용

### 검색 플로우
```mermaid
graph TD
    A[검색 요청] --> B{LangChain 사용 가능?}
    B -->|Yes| C[LangChain 벡터 검색]
    C --> D[기존 KeywordSearchService 검색]
    D --> E[수동 하이브리드 융합 + 중복 제거]
    E --> F[결과 반환]
...
```bash
# 환경변수
OPENAI_API_KEY=your_openai_api_key  # 필수: OpenAI text-embedding-3-small 및 LangChain용
PINECONE_API_KEY=your_pinecone_api_key
PINECONE_INDEX_NAME=resume-vectors  # 1536차원으로 생성 필요
...
**⚠️ 중요**: Pinecone 인덱스를 1536차원으로 재생성해야 합니다.
...
### Vector Service 기능 (업그레이드됨) 🆕
- ✅ **OpenAI text-embedding-3-small** 기반 고품질 벡터 변환 (1536차원)
- ✅ **폴백 지원**: OpenAI 실패 시 SentenceTransformer 자동 사용
- ✅ 벡터 기반 의미적 유사도 검색
- ✅ 다차원 벡터 공간에서의 문서 검색 
- ✅ 메타데이터 기반 필터링 지원
- ✅ **LangChain 호환**: 동일한 임베딩 모델로 일관성 보장
...
### 키워드 검색 서비스 기능 🆕 (Elasticsearch + LangChain 기반)
- ✅ **LangChain EnsembleRetriever** 통합 - 고도화된 하이브리드 검색
...
- ✅ **LangChain 하이브리드 검색** (EnsembleRetriever 우선, 폴백 지원)
- ✅ **확장성** (분산 검색, 샤딩 지원)
