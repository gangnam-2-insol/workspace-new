...
const FloatingChatbot = ({ page, onFieldUpdate, onComplete, onPageAction }) => {
  ...
  useEffect(() => {
    ...
    // Ctrl+F5 또는 F5로 강제 새로고침된 경우
    if (isHardRefresh) {
      console.log('🔄 강제 새로고침 감지됨 - 세션 완전 초기화');
      // 세션 완전 초기화
      sessionStorage.removeItem('chatbotWasOpen');
      sessionStorage.removeItem('chatbotMessages');
      sessionStorage.removeItem('shouldResetSession');
    ...
    // beforeunload 이벤트로 일반 새로고침 감지
    const handleBeforeUnload = () => {
      sessionStorage.setItem('shouldResetSession', 'true');
      console.log('🔄 페이지 새로고침 감지 - 세션 초기화 예정');
    };
  ...
  // 일반 페이지 변경 시 챗봇 상태 유지 (새로고침이 아닌 경우)
  useEffect(() => {
    // 세션 초기화 플래그가 없을 때만 페이지 변경 처리
    const shouldReset = sessionStorage.getItem('shouldResetSession');
    if (shouldReset !== 'true') {
      console.log('📄 일반 페이지 변경됨:', page, '챗봇 상태 유지');
      
      // AI 모드 관련 상태만 초기화 (페이지별로 다른 컨텍스트이므로)
      setAiMode(false);
      setAiStep(1);
      setAiFormData({
        department: '',
      ...
      // 페이지 변경 시 안내 메시지 추가 (기존 대화는 유지, 중복 방지)
      if (isOpen && messages.length > 0) {
        // 마지막 메시지가 이미 페이지 변경 알림인지 확인
        const lastMessage = messages[messages.length - 1];
        const isAlreadyPageInfo = lastMessage && 
          lastMessage.type === 'bot' && 
          lastMessage.content.includes('페이지 변경 알림');
        
        if (!isAlreadyPageInfo) {
          const pageInfoMessage = {
            type: 'bot',
            content: `📍 **페이지 변경 알림**\n\n현재 ${getPageTitle(page)} 페이지에 있습니다. 이전 대화 내용은 그대로 유지됩니다.\n\n${getWelcomeMessage(page).split('📈')[0]}`, // 환영 메시지의 첫 부분만 사용
            timestamp: new Date()
          };
          setMessages(prev => [...prev, pageInfoMessage]);
        }
      }
    }
  }, [page, isOpen, messages]); // 필요한 의존성 추가

  // 환영 메시지 설정을 위한 별도 useEffect (세션 초기화 시에만 새로운 환영 메시지)
  useEffect(() => {
    // 세션 초기화 플래그가 없고, 메시지가 없을 때만 환영 메시지 추가
    const shouldReset = sessionStorage.getItem('shouldResetSession');
    if (shouldReset !== 'true' && messages.length === 0) {
      const welcomeMessage = {
        type: 'bot',
        content: getWelcomeMessage(page),
        timestamp: new Date()
  ...
  }, [page, messages.length]); // page 변경 시에만 실행
  ...
  const getPageTitle = (currentPage) => {
    ...
    return pageTitles[currentPage] || pageTitles['default'];
  };
